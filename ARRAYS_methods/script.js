...
for (let index = 0; index < bigdata.data.length; index++) {
    const element = bigdata.data[index];
    console.log(`id: ${element.id}, name: ${element.name}, min-size: ${element.min_size}`);
}


for (const property in bigdata) {
  for (const iterator of bigdata[property]) {
    // console.log(typeof iterator.id)
    console.log(`id: ${iterator.id}, name: ${iterator.name}, min-size: ${iterator.min_size}`);
  }
}
